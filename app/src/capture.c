...
static void sendResponseT(const void *str, int size)
{
...
static void closeConnectionT(void)
{
...
void capture_init(void)
{
    ...
    init_device();
    start_capturing();
    if (pthread_create(&thread_id, NULL, mainloop, NULL) != 0) {
...
void capture_cleanup(void) 
{
    ...
    if (pthread_join(thread_id, NULL) != 0) {
        perror("failed to end capture thread");
        exit(EXIT_FAILURE);
    }
    stop_capturing();
    uninit_device();
